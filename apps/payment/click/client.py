...
class ClickUPClient:
    ...
    def create_payment_link(cls, transaction: Transaction):
        ...
        return_url = credentials["CLICK_REDIRECT_URL"]
        ...
        if return_url:
            parsed_return_url = urllib.parse.quote(return_url + f"?transaction_id={transaction.id}")
            query += f"&return_url={parsed_return_url}"
